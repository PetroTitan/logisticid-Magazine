 ...
 * at once.
 *
 * `publicArticles("ru")` means the params are Russian articles ONLY. Combined
 * with `dynamicParams = false`, an English slug under `/ru/` is a real 404
 * rather than an English article served from a Russian URL with a 200 — which
 * is the duplicate-content failure the whole route split exists to prevent.
 */
export function generateStaticParams() {
  return publicArticles("ru").map((article) => ({
...
export async function generateMetadata({ params }: Params) {
  ...
  const article = findArticle(section, slug, "ru");
...
export default async function RussianArticlePage({ params }: Params) {
  const { section, slug } = await params;
  const article = findArticle(section, slug, "ru");
  if (article === undefined) notFound();
  return <ArticleView article={article} locale="ru" />;
}
